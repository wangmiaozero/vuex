...
var store = new Vuex.Store({
  state:{
    //state  可以说是组件中的data,专门来用来存储数据的
    // 如果在 组件中，想要访问，store 中的数据，只能通过 this.$store.state.*** 来访问
    count:0
  },
  mutations:{
      // 注意： 如果要操作 store 中的 state 值，
      //只能通过 调用 mutations 提供的方法，才能操作对应的数据，
      //不推荐直接操作 state 中的数据，因为 万一导致了数据的紊乱，
  ...
  getters:{
    ...
    optCount:function(state){
     ...
     // 经过咱们回顾对比，发现 getters 中的方法，
     // 和组件中的过滤器比较类似，因为 过滤器和 getters 都没有修改原数据， 都是把原数据做了一层包装，提供给了 调用者；
